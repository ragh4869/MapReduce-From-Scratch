...
class Master:
    ...
    def check_mapper_reducer_completion(self, check_list):
        c = 0
        not_done = []
        while check_list:
    ...
    def file_chunk(self, file_size, flag, mapper_count):
        ...
        # Condition if the file sent is a folder
            ...
            file_count = len([j for j in os.listdir(self.filename) if '.txt' in j]) # Get file count
            for i in [j for j in os.listdir(self.filename) if '.txt' in j]:
                ...
                if round(no_of_splits) == 0 or no_of_splits == 1:
                    ...
                    f_size = file_size//round(no_of_splits) + 1
                    with open(self.filename+"//"+i, encoding='utf-8') as f:
    ...
    def split_file(self):
        ...
        self.file_chunk(file_size, flag, self.mappers)
    ...
    def run_mapper(self, mapper_barrier, file_name):
        
        if self.mapper_func == 'map_wc':
            mapper_process = Process(target=map_wc, args=(mapper_barrier, file_name,))
            mapper_process.start()
        elif self.mapper_func == 'map_inv_ind':
            mapper_process = Process(target=map_inv_ind, args=(mapper_barrier, file_name,))
            mapper_process.start()
    
    # Run the reducer using Process 
    def run_reducer(self, reducer_barrier, file_name):
        
        if self.reducer_func == 'red_wc':
            reducer_process = Process(target=red_wc, args=(reducer_barrier, file_name,))
            reducer_process.start()
        elif self.reducer_func == 'red_inv_ind':
            reducer_process = Process(target=red_inv_ind, args=(reducer_barrier, file_name,))
            reducer_process.start()
...
def server_program(task, filename, mappers, reducers, mapper_func, reducer_func):
    ...
    for i in M.split_files:
        ...
        M.mapper_list.append("map_"+i) # Appending mapper list
    ...
    mapper_list = M.mapper_list.copy() # Getting the list of json files to check for
    ...
    while max_tries:
        ...
        check,not_done = M.check_mapper_reducer_completion(mapper_list)
        ...
        if check == M.mappers:
            ...
            new_mapper_barrier = Barrier(check+1)
            # Running only the files which were not successfully completed
            for i in not_done:

                # Delete respective files before restarting
                comm = f"del {i}\n"
                M.call_set_del_write_file(comm)

                file_val = i.replace('map_','')
                M.run_mapper(new_mapper_barrier, file_val)
    ...
    while max_tries:
        # Running the check function
        check,not_done = M.check_mapper_reducer_completion(reducer_list)
        ...
        if check == M.reducers:
            ...
            new_reducer_barrier = Barrier(check+1)
            # Running only the files which were not successfully completed
            for i in not_done:

                # Delete respective files before restarting
                file_val = f"{M.task}\\"+i.replace('.json', '_out.json')
                comm = f"del {i}\n"
                M.call_set_del_write_file(comm)

                file_val = f"{M.task}\\"+i
                M.run_reducer(new_reducer_barrier, file_val)
